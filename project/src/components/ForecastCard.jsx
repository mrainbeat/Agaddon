...
function buildForecastMessage({ nextSub, budget }) {
  ...
  if (nextSub && nextSub.daysLeft <= 0) {
    return `오늘 <strong class="text-[#FF795B]">${nextSub.name} (<span class="text-[22px] font-bold">${formatWon(nextSub.amount)}</span>)</strong> 결제일이야!`;
  }
  ...
  if (ratio >= 0.7) {
    ...
    if (cushion > 0) {
      return `<strong class="text-[#FF795B]"><span class="text-[22px] font-bold">${formatWon(cushion)}</span></strong> 더 쓰면 하루를 <strong class="text-[#FF795B]">${SURVIVAL_ITEM_LABEL}</strong>로 버텨야 해ㅜ`;
    }
    return `지금부터는 하루 <strong class="text-[#FF795B]">${SURVIVAL_ITEM_LABEL}(<span class="text-[22px] font-bold">${formatWon(SURVIVAL_DAILY_AMOUNT)}</span>)</strong> 수준으로만 버틸 수 있어... 오늘은 진짜 아껴줘ㅠㅠ`;
  }

  if (nextSub && nextSub.daysLeft <= 3) {
    return `${nextSub.daysLeft}일 뒤에 <strong class="text-[#FF795B]">${nextSub.name} (<span class="text-[22px] font-bold">${formatWon(nextSub.amount)}</span>)</strong> 결제가 있어!`;
  }
  ...
  return (
    <div className="flex flex-col items-center gap-0">
      <div className="relative max-w-[300px] rounded-2xl bg-[#FFE3CC] px-[17px] py-3 text-center after:absolute after:-bottom-2 after:left-1/2 after:border-8 after:border-transparent after:border-b-0 after:border-t-[#FFE3CC] after:content-[''] after:[transform:translateX(-50%)]">
        <p
          className="text-[18px] font-semibold leading-normal tracking-[-0.18px] text-[#6B493D] break-keep"
