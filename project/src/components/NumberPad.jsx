...
function NumberPad({ onDigit, onBackspace, maxLength = 10 }) {
  return (
    <div className="number-pad">
      {KEYS.map((key, i) => {
        if (key === '') return <div key={`empty-${i}`} className="pad-key pad-key-empty" aria-hidden="true" />

        if (key === 'back') {
          return (
            <button key="back" type="button" className="pad-key pad-key-back" onClick={onBackspace} aria-label="지우기">
